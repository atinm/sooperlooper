...
Engine::process (nframes_t nframes)
{
  ...
  if (num > 0) {
    ...
    while (evt)
      {
        LOG("evt->Type:", evt->Type, "evt->Command:", evt->Command, "evt->Control:", evt->Control, "evt->Instance:", ((int)evt->Instance));
...
Engine::do_global_rt_event (Event * ev, nframes_t offset, nframes_t nframes)
{
  ...
  if (ev->Control == Event::TapTempo) {
	...
	{
	  LOG("target_instance: ", target_instance, " _rt_instance.size(): ", _rt_instances.size());
